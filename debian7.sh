...
# set repo
wget -O /etc/apt/sources.list "https://raw.githubusercontent.com/brantbell/wulandari/srie/repo/sources.list.debian8"
...
sed -i 's/DROPBEAR_EXTRA_ARGS=/DROPBEAR_EXTRA_ARGS="-p 442" -b /etc/bannerssh"/g' /etc/default/dropbear
...
echo "ADMIN WWW KOPET.COM" | tee -a log-install.txt
